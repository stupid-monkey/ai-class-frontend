...
const designStyle = c.indexOf(`                    <el-form-item :label="$t('dashboard_mod.designStyle')">`);
const endTmpl = c.indexOf('</el-button>', designStyle) + '</el-button>'.length;
...
c = c.substring(0, designStyle) + newBtnsHTML + c.substring(endTmpl);

// 2. Replace Credits View
const btnDlStart = c.indexOf(`<a :href="pptResultUrl" target="_blank" style="text-decoration: none;">`);
const btnDlEndDiv = c.indexOf(`</div>`, c.indexOf(`</div>`, btnDlStart) + 6) + 6;
...
// 3. Replace Function generatePPT
