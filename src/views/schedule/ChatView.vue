    ...
    <main class="form-area">
            ...
            <el-row :gutter="20">
              <el-col :span="12">
                <el-form-item label="任务名称">
                  <el-input
                    ...
                    placeholder="例如：2026年春季高一(1)班排课测试"
                  />
                </el-form-item>
              </el-col>
              <el-col :span="12">
              ...
              <el-col :span="6"
                ><el-form-item label="上午节数"
                  ><el-input-number
                    v-model="formData.morningPeriods"
                    :min="0"
                    :max="10"
                    style="width: 100%" /></el-form-item
              ></el-col>
              <el-col :span="6"
                ><el-form-item label="下午节数"
                  ><el-input-number
                    v-model="formData.afternoonPeriods"
                    :min="0"
                    :max="10"
                    style="width: 100%" /></el-form-item
              ></el-col>
          ...
          <el-card shadow="never" class="box-card" style="margin-top: 20px">
            <template #header>
              <div style="display: flex; justify-content: space-between; align-items: center">
                ...
                <el-button type="primary" plain size="small" @click="addRequirement"
                  >+ 添加一条需求</el-button
                >
              </div>
            </template>
            <div v-if="formData.requirements.length === 0" class="empty-tip">
              暂无排课需求，请点击右上角添加。
            </div>
            ...
            >
              <el-col :span="7"
                ><el-input v-model="item.classId" placeholder="班级ID (如: CLS_001)"
              /></el-col>
              <el-col :span="7"
                ><el-input v-model="item.courseName" placeholder="学科名称 (如: 语文)"
              /></el-col>
              <el-col :span="6"
                ><el-input-number
                  v-model="item.weeklyCount"
                  :min="1"
                  :max="20"
                  style="width: 100%"
                  placeholder="周课时"
              ...
              <el-col :span="4" style="text-align: right"
                ><el-button type="danger" circle icon="Delete" @click="removeRequirement(index)"
              /></el-col>
            </el-row>
          </el-card>

          <div class="submit-action">
            <el-button type="success" size="large" @click="submitForm" :loading="isSubmitting"
              >确认无误，开始智能排课</el-button
            >
          </div>
        </el-form>
      </div>
      ...
      <div v-else class="result-area">
        <el-result
          icon="success"
          title="任务已提交"
          sub-title="校历生成完毕。请在下方点击任意日期，AI 将为您结算该周课表"
        >
          <template #extra>
...
const getSubjectIds = (courseName: string) => {
  const map: Record<string, { cId: string; tId: string }> = {
    语文: { cId: 'C_CHINESE', tId: 'T_CHI' },
    数学: { cId: 'C_MATH', tId: 'T_MAT' },
    英语: { cId: 'C_ENGLISH', tId: 'T_ENG' },
    物理: { cId: 'C_PHYSICS', tId: 'T_PHY' },
    体育: { cId: 'C_PE', tId: 'T_PE' },
    美术: { cId: 'C_ART', tId: 'T_ART' },
    历史: { cId: 'C_HISTORY', tId: 'T_HIS' },
    政治: { cId: 'C_POLITICS', tId: 'T_POL' },
    生物: { cId: 'C_BIOLOGY', tId: 'T_BIO' },
    化学: { cId: 'C_CHEMISTRY', tId: 'T_CHE' },
  }
  for (const key in map) {
    if (courseName.includes(key)) return map[key]
  }
...
onMounted(() => {
  ...
  if (cachedData)
    try {
      Object.assign(formData, JSON.parse(cachedData))
    } catch (e) {}
})
watch(
  formData,
...
const addRequirement = () => {
  formData.requirements.push({ classId: 'CLS_001', courseName: '', weeklyCount: 1 })
}
...
const submitForm = async () => {
  ...
  const submitPayload = {
    ...
    requirements: formData.requirements.map((req) => {
      const mappedIds = getSubjectIds(req.courseName) || {
        cId: `C_CUSTOM_${req.courseName}`,
        tId: `T_CUSTOM_${req.courseName}`,
      }
      return {
        classId: req.classId,
        courseId: mappedIds.cId,
        teacherId: mappedIds.tId,
        weeklyCount: req.weeklyCount,
      }
    }),
  }
...
const startPollingWeeklyData = () => {
  ...
  pollTimer1 = setInterval(async () => {
    try {
      const weekRes = await getScheduleResultApi(
        currentTaskId.value,
        currentClassId.value,
        clickedDateString.value,
      )
