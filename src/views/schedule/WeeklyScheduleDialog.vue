<template>
  ...
  >
    <div
      v-loading="isLoadingWeeklyData"
      :element-loading-text="pollingStatusText"
      class="dialog-inner-content"
    >
            ...
            <el-table-column prop="periodNum" label="节次" width="80" align="center" fixed="left">
              <template #default="scope">
                ...
                <div style="font-size: 12px; color: #909399; margin-top: 4px">
                  {{ scope.row.periodNum <= baseConfig.morningPeriods ? '上午' : '下午' }}
                </div>
              </template>
            </el-table-column>

            <el-table-column
              v-for="day in baseConfig.daysPerWeek"
              :key="day"
              align="center"
              min-width="120"
            >
              ...
              <template #default="scope">
                <div
                  v-if="scope.row['day' + day]"
                  class="course-cell"
                  :style="{
                    backgroundColor: getCourseColor(scope.row['day' + day].courseName).bg,
                    borderColor: getCourseColor(scope.row['day' + day].courseName).border,
                  }"
                >
                  <div
                    class="course-name"
                    :style="{ color: getCourseColor(scope.row['day' + day].courseName).text }"
                  >
                    {{ scope.row['day' + day].courseName }}
                  </div>
        ...
        <el-card shadow="never" class="box-card chat-card">
          ...
          <div class="chat-input-area">
            ...
            <el-button
              ...
              style="margin-left: 12px; border-radius: 8px"
              >发送</el-button
            >
          </div>
        </el-card>
    ...
    >
      <div v-if="evaluationData">
        <div class="summary-comment">
          ...
          <div>
            <strong>综合评价：</strong><br />{{ evaluationData.evaluations[0].summaryComment }}
          </div>
        </div>
        ...
        <el-table :data="evaluationData.evaluations[0].deductionItems" border stripe>
          <el-table-column prop="dimension" label="评估维度" width="120" align="center">
            <template #default="scope"
              ><el-tag type="warning">{{ scope.row.dimension }}</el-tag></template
            >
          </el-table-column>
          ...
          <el-table-column prop="deduction" label="扣分" width="80" align="center">
            <template #default="scope"
              ><span style="color: #f56c6c; font-weight: bold"
                >-{{ scope.row.deductScore || scope.row.deduction }}</span
              ></template
            >
          </el-table-column>
        </el-table>
    ...
    try {
      ...
      if (currentStatus === 'SUCCESS') {
        ...
        chatMessages.value = [
          {
            role: 'ai',
            content: `这是 ${props.dateString} 所在周的课表。如需对本周微调，请直接告诉我。`,
          },
        ]
...
// ============= 颜色与矩阵转换工具 =============
const colorPalette = [
  { bg: '#ecf5ff', text: '#409eff', border: '#d9ecff' },
...
const getCourseColor = (courseName: string) => {
  if (!courseName || courseName === '无')
    return { bg: '#ffffff', text: '#909399', border: '#ebeef5' }
...
const sendChatMessage = async () => {
  ...
  try {
    ...
    const replyText =
      typeof aiReplyMsg === 'string'
        ? aiReplyMsg
        : typeof aiReplyMsg === 'object' && aiReplyMsg && 'data' in aiReplyMsg
          ? aiReplyMsg.data
          : '收到指令，课表已调整。'
