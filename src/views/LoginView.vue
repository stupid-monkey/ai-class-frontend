  ...
  <div class="login-wrapper">
    ...
    <el-card class="login-card" shadow="hover">
      ...
      <el-form :model="loginForm" @keyup.enter="handleLogin" label-width="0">
        <el-form-item v-if="activeRole !== 'admin'">
          <el-input
            v-model="loginForm.schoolCode"
            :placeholder="$t('login.schoolCodePlaceholder') || '请输入学校编码'"
            size="large"
            prefix-icon="School"
            :disabled="isLoading"
            clearable
        ...
        >
          {{
            isLoading
              ? $t('common.loading')
              : activeRole === 'teacher'
                ? $t('login.loginBtnTeach')
                : $t('login.loginBtnStud')
          }}
        </el-button>
      </el-form>
    </el-card>
...
onMounted(() => {
  ...
  const savedSchoolCode = sessionStorage.getItem('lastSchoolCode')
  ...
  if (savedSchoolCode) {
    loginForm.schoolCode = savedSchoolCode
  }
...
const handleLogin = async () => {
  // 验证输入
  if (activeRole.value !== 'admin' && !loginForm.schoolCode.trim()) {
    ElMessage.error(t('login.schoolCodePlaceholder') || '请输入学校编码')
    return
  }
  ...
  try {
    ...
    if (activeRole.value !== 'admin') {
      payload.schoolCode = loginForm.schoolCode
    }
    ...
    const res = await loginApiCall(payload)
    ...
    if (code === 200 || code === 0) {
      ...
      if (activeRole.value !== 'admin') {
        sessionStorage.setItem('lastSchoolCode', loginForm.schoolCode)
      }
      ...
      if (data.mustChangePassword) {
        ...
        // 检查是否有重定向路径
        const redirectPath =
          sessionStorage.getItem('redirectPath') || (route.query.redirect as string)
